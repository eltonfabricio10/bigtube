...
- **Búsqueda de YouTube integrada** - Busca sin abrir un navegador, con filtro de tipo: **Videos**, **En vivo**, **Canales** o **Listas de reproducción**
...
- **Listas de reproducción por enlace** - Pega un enlace de una lista de reproducción de YouTube (`playlist?list=` o `watch?v=...&list=`) y la búsqueda mostrará hasta los primeros 500 videos (límite para mantener ágiles las listas muy grandes)
...
| `-o, --output DIR` | Carpeta de destino solo para este `--download` (predeterminado: carpeta configurada; no cambia la configuración de la interfaz) |
...
| **Acelerar con aria2c** | Activado | Usa `aria2c` como descargador externo de `yt-dlp` para descargas más rápidas con múltiples conexiones y reanudables. Solo se aplica cuando `aria2c` está instalado; de lo contrario, la descarga es normal. |
...
| `use_aria2c` | `true` | Usa `aria2c` como descargador externo cuando está instalado |
